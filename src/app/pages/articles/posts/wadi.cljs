  ...
  [:div.container.about
   ...
   [:button.button.is-clickable.is-rounded.is-info.is-light
    {:style {:margin-left "4.5rem"}
     :on-click #(rf/dispatch [:app.events/set-route-params {:language "ar"}])} "اقرأ باللغة العربية"]
  ...
  [:div.container.about  {:dir "rtl" :lang "ar" :style {:font-size "1.4rem"}}
   ...
   [:button.button.is-clickable.is-rounded.is-info.is-light
    {:style {:margin-right "4.5rem"}
     :on-click #(rf/dispatch [:app.events/set-route-params {:language "en"}])} "Read in English"]
